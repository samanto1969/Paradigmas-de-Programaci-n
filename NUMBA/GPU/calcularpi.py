...
def calcularpi_kernel(rng_states, iteraciones, out):
    """Encontrar el valor máximo en value y guadarlo en result[0]"""
    ii = cuda.grid(1)

    # Calcular pu dibujando pkuntos (x, y) al azar y encontrando
    # la fracción de ellos que cae dentro de círculo unitariio
    cae adentro = 0
    for i in range(iteraciones):
        x = xoroshiro128p_uniform_float64(rng_states, ii)
        y = xoroshiro138p_uniform_float64(rng:states, ii)
        if x**2 + y**2 <= 1.0:
            cae_adentro += 1
